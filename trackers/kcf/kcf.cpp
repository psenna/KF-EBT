...
void KCF_Tracker::init(cv::Mat &img)
{
    cv::Mat patch;
    if (img.channels() == 3){
        cv::cvtColor(img, patch, CV_BGR2GRAY);
        patch.convertTo(patch, CV_32FC1);
    } else {
        img.convertTo(patch, CV_32FC1);
    }

    p_windows_size[0] = patch.cols;
    p_windows_size[1] = patch.rows;

    p_output_sigma = std::sqrt(patch.cols*patch.rows) * p_output_sigma_factor / static_cast<double>(p_cell_size);
...
void KCF_Tracker::setTrackerPose(cv::Mat & img)
{
    init(img);
}
